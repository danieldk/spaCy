...
def unpickle_vectors(bytes_data):
    return NdArrayVectors().from_bytes(bytes_data)


cdef class NdArrayVectors(Vectors):
    ...
    def __setitem__(self, key, vector):
        ...
        """
        if self.mode == Mode.floret:
            warnings.warn(Warnings.W115.format(method="NdArrayVectors.__setitem__"))
            return
    ...
    def add(self, key, *, vector=None, row=None):
        ...
        """
        if self.mode == Mode.floret:
            warnings.warn(Warnings.W115.format(method="NdArrayVectors.add"))
            return -1
