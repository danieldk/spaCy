...
def build_tagger_model_v2(
    ...
    normalize (bool): Normalize the softmax layer outputs.
    """
...
@registry.architectures("spacy.Tagger.v3")
def build_tagger_model_v3(
    tok2vec: Model[List[Doc], List[Floats2d]],
    nO: Optional[int] = None,
    normalize: bool = False,
    ...
    nO (int or None): The number of tags to output. Inferred from the data if None.
    normalize (bool): Normalize the softmax layer outputs.
    temperature (float): Temperature used by the softmax layer.
    """
    # TODO: glorot_uniform_init seems to work a bit better than zero_init here?!
    t2v_width = tok2vec.get_dim("nO") if tok2vec.has_dim("nO") else None
    output_layer = Softmax_v2(
