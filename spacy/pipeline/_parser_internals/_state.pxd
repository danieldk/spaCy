...
cdef cppclass StateC:
    ...
    void get_arcs_(const unordered_map[int, vector[ArcC]] &heads_arcs, vector[ArcC]* out) nogil const:
    ...
    void get_arcs(vector[ArcC]* out) nogil const:
        this.get_arcs_(this._left_arcs, out)
        this.get_arcs_(this._right_arcs, out)
    ...
    int head_arc(const unordered_map[int, vector[ArcC]]& heads_arcs, int head, int idx) nogil const:
        if idx < 1:
            return -1
    ...
    int L(int head, int idx) nogil const:
        return this.head_arc(this._left_arcs, head, idx)

    int R(int head, int idx) nogil const:
        return this.head_arc(this._right_arcs, head, idx)
    ...
    int n_arcs(const unordered_map[int, vector[ArcC]] &heads_arcs, int head) nogil const:
        ...
        while arcs_it != arcs.end():
            arc = deref(arcs_it)
            if arc.head == head and arc.child != -1:
                n += 1
            incr(arcs_it)
    ...
    void del_arc_(unordered_map[int, vector[ArcC]]* heads_arcs, int h_i, int c_i) nogil:
    ...
    void del_arc(int h_i, int c_i) nogil:
        if h_i > c_i:
            this.del_arc_(&this._left_arcs, h_i, c_i)
        else:
            this.del_arc_(&this._right_arcs, h_i, c_i)
