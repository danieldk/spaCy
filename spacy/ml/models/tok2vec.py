...
def build_hash_embed_cnn_tok2vec_ragged(
    *,
    width: int,
    depth: int,
    embed_size: int,
    ...
    if subword_features:
        ...
        row_sizes = [embed_size]
    return build_Tok2Vec_model(
        embed=MultiHashEmbedRagged(
            width=width,
            rows=row_sizes,
            attrs=attrs,
            include_static_vectors=bool(pretrained_vectors),
        ),
        encode=MaxoutWindowEncoderRagged(
            width=width,
            depth=depth,
            window_size=window_size,
            maxout_pieces=maxout_pieces,
